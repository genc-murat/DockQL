...
use bollard::models::{
    ContainerInspectResponse, ContainerSummary, EventMessage, ImageSummary, PortSummary,
    Volume as BollardVolume,
};
...
pub trait DockerClient {
    fn list_containers(
        &self,
    ) -> impl std::future::Future<Output = Result<Vec<Container>, DockerError>> + Send;
    fn list_images(
        &self,
    ) -> impl std::future::Future<Output = Result<Vec<Image>, DockerError>> + Send;
    fn list_networks(
        &self,
    ) -> impl std::future::Future<Output = Result<Vec<Network>, DockerError>> + Send;
    fn list_volumes(
        &self,
    ) -> impl std::future::Future<Output = Result<Vec<Volume>, DockerError>> + Send;
    fn inspect_container(
        &self,
        id: &str,
    ) -> impl std::future::Future<Output = Result<Container, DockerError>> + Send;
    fn container_logs(
        &self,
        id: &str,
        tail: usize,
    ...
    fn container_stats(
        &self,
        id: &str,
    ) -> impl std::future::Future<Output = Result<MetricSample, DockerError>> + Send;
    fn events_stream(
        &self,
        since: Option<&str>,
        until: Option<&str>,
    ) -> impl std::future::Future<Output = Result<DockerEventStream, DockerError>> + Send;
...
async fn docker_call_with_retry<T, F, Fut>(cfg: &DockerApiConfig, f: F) -> Result<T, DockerError>
where
    F: Fn() -> Fut,
...
pub async fn list_running_containers<C: DockerClient>(
    client: &C,
...
impl BollardDockerClient {
    ...
    pub fn connect_with_config(config: &DolConfig) -> Result<Self, DockerError> {
        let docker =
            bollard::Docker::connect_with_local_defaults().map_err(DockerError::Bollard)?;
        Ok(Self {
            docker,
...
impl DockerClient for BollardDockerClient {
    async fn list_containers(&self) -> Result<Vec<Container>, DockerError> {
        ...
        if !containers.is_empty() {
            ...
            for (i, id) in ids.iter().enumerate() {
                if let Ok(inspect) = docker_call_with_retry(&self.api_cfg, || {
                ...
                .await
                {
                    ...
                    containers[i].finished_at = s
                        ...
                        .filter(|ts| !ts.is_empty() && !ts.starts_with("0001"));
                    containers[i].restart_count = inspect.restart_count.map(|c| c.max(0) as u64);
                    containers[i].health = s
                        .and_then(|st| st.health.as_ref())
                        .map(|h| h.status.map(|e| format!("{e:?}")).unwrap_or_default())
                        .filter(|s| !s.is_empty());
                }
            }
        }
    ...
    async fn inspect_container(&self, id: &str) -> Result<Container, DockerError> {
        let inspect =
            docker_call_with_retry(&self.api_cfg, || self.docker.inspect_container(id, None))
                .await?;
    ...
    async fn container_stats(&self, id: &str) -> Result<MetricSample, DockerError> {
        ...
        match next {
            Ok(Some(Ok(stats))) => {
                let container_name = docker_call_with_retry(&self.api_cfg, || {
                    self.docker.inspect_container(id, None)
                })
                .await
                .ok()
                ...
                Ok(metric_sample_from_bollard_stats(
                    id,
                    &container_name,
                    &stats,
                ))
...
fn container_from_inspect(inspect: &ContainerInspectResponse) -> Container {
    ...
    Container {
                ...
                ports_obj
                    ...
                    .flat_map(|(container_port, bindings)| {
                        ...
                        if arr.is_none_or(std::vec::Vec::is_empty) {
                            ...
                            arr.expect("bindings should be Some here (checked by is_none_or)")
                                .iter()
                                .map(|b| {
        ...
        labels: config
            ...
            .map(|l| labels_map_to_vec(Some(l)))
            .unwrap_or_default(),
...
fn image_from_summary(s: &ImageSummary) -> Image {
    let id = s.id.clone();
    let (repository, tag) = s.repo_tags.first().map_or_else(
        || (id.clone(), "latest".to_owned()),
        |full| {
            if let Some((repo, t)) = full.rsplit_once(':') {
                (repo.to_owned(), t.to_owned())
            } else {
                (full.clone(), "latest".to_owned())
            }
        },
    );
    let repository = if repository == "<none>" {
        id.clone()
    } else {
        repository
    };
...
#[must_use]
pub fn metric_sample_from_bollard_stats(
    container_id: &str,
    container_name: &str,
    ...
    let cpu_percent = stats.cpu_stats.as_ref().and_then(|cpu| {
        ...
        if system > 0 && total > 0 {
            Some((total as f64 / system as f64) * f64::from(cpus) * 100.0)
        } else {
            None
        }
    });
            ...
            p.iter()
                .map(|port| {
                    let typ = port
                        .typ
                        .as_ref()
                        .map_or_else(|| "tcp".to_owned(), |e| format!("{e:?}").to_lowercase());
                    port.public_port.map_or_else(
                        || format!("{}/{}", port.private_port, typ),
                        |host_port| {
                            format!(
                                "{}:{}->{}/{}",
                                port.ip.as_deref().unwrap_or("0.0.0.0"),
                                host_port,
                                port.private_port,
                                typ
                            )
...
fn unix_to_iso(unix: i64) -> String {
    ...
    chrono::Utc
        ...
        .single()
        .map_or_else(|| unix.to_string(), |dt| dt.to_rfc3339())
}
...
fn string(value: &serde_json::Value, keys: &[&str]) -> String {
...
mod tests {
    ...
    fn mock_inspect_not_found() {
        let err = rt()
            .block_on(MockDockerClient::default().inspect_container("x"))
            .unwrap_err();
